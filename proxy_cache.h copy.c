...
void handle_connection(int connfd, struct request_info *r_info, char *header_buf)
{
  ...
  while (1)
  {
    ...
    {
      void *ptr = (void *)((char *)(cache_content_buf) + object_total_byte_count - byte_count);
             ...
             char *version, struct request_info *r_info)
{
  ...
  {
    clienterror(fd, method, "501", "Not Supported method", "Proxy does not support this method");
...
void read_requesthdrs(rio_t *rp, char *header_buf)
{
  ...
  while (strcmp(buf, "\r\n") && read_count > 0)
  {
    ...
    if (strstr(buf, "Connection:") || strstr(buf, "Proxy-Connection") || strstr(buf, "User-Agent") || strstr(buf, "Host:"))
    { // do nothing
    }
    else
    {
